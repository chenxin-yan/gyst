...
import type { ApplyEnvelope, ApplyOp, Hunk, Session } from "./index.ts";
import { applyBatch } from "./index.ts";
...
const applied = (ops: ApplyOp[], from = session, idempotencyKey?: string) =>
  Result.getOrThrow(applyBatch(from, batch(ops, idempotencyKey))).session!;
const rejected = (ops: ApplyOp[]) => {
  const result = applyBatch(session, batch(ops));
...
describe("applyBatch", () => {
  ...
  it("replays only an identical envelope under a reused idempotency key", () => {
    ...
    const first = Result.getOrThrow(applyBatch(session, batch(ops)));
    const next = first.session!;

    const replay = Result.getOrThrow(applyBatch(next, batch(ops)));
    ...
    for (const different of [
      ...
      const reused = applyBatch(next, different);
