...
export function daemonTuiClient(cwd = process.cwd()): TuiClient {
  function errorPayload(error: unknown): ErrorPayload {
    ...
    catch { return { code: "daemon_unreachable", message: error instanceof Error ? error.message : "invalid daemon response" }; }
  }
  ...
  return {
    status: async () => Schema.decodeUnknownSync(StatusPayloadSchema)(await send({ command: "status", cwd, args: [] })),
    diff: async () => Schema.decodeUnknownSync(DiffPayloadSchema)(await send({ command: "diff", cwd, args: [] })),
    action: async (action) => Schema.decodeUnknownSync(StatusPayloadSchema)(await send({ command: "tui.action", cwd, args: [], action })),
    refresh: async () => Schema.decodeUnknownSync(StatusPayloadSchema)(await send({ command: "refresh", cwd, args: [] })),
  };
}
