...
const patch = (file: string, from: string, to: string) => ({
  id: file, file, header: "-1 +1", contentHash: file, accepted: false,
  patch: `@@ -1 +1 @@\n-${from}\n+${to}`,
});
...
const state = fixture();
const tui = await testRender(() => <App client={state.client} pollInterval={60_000} />, { width: 100, height: 30 });
...
await press(tui, "0");
tui.resize(140, 30); await tui.renderOnce();
...
const waitingTui = await testRender(() => <App client={waiting} pollInterval={5} onQuit={() => { quit = true; }} />, { width: 100, height: 30 });
...
const resetTui = await testRender(() => <App client={resetState.client} pollInterval={5} />, { width: 100, height: 30 });
await resetTui.waitForFrame((value) => value.includes("✓ accepted"));
const reset = structuredClone(resetState.status()) as any;
reset.groups[0].accepted = false; reset.revision = 2;
