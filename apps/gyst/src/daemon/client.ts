import { ReplySchema, type ErrorPayload, type Reply, type Request } from "@gyst/core";
    ...
    void Bun.connect<{ request: Request }>({
      ...
      socket: {
        ...
        data(_socket, bytes) {
          ...
          if (newline < 0) return;
          try {
            resolve(Schema.decodeUnknownSync(ReplySchema)(JSON.parse(buffer.slice(0, newline))));
          } catch (error) {
            fail(error);
          }
        },
...
export async function requestDaemon(request: Request): Promise<Reply> {
  try {
    ...
    if (!(error instanceof ExchangeError) || error.requestSent)
      throw unreachable("daemon response was lost; request was not retried", error);
  }
  try {
