...
async function gitPatch(
  ...
  const diff = Bun.spawnSync(["git", "diff", ...args], {
    cwd: root,
    stdout: "pipe",
    stderr: "pipe",
  });
...
async function handle(request: Request): Promise<StatusPayload | ClosePayload | DiffPayload> {
  if (request.command === "create") {
    ...
    const patch = values.stdin
      ? (request.stdin ?? "")
      : await gitPatch(root, gitArgs, includeUntracked);
    ...
    const session: Session = {
      ...
      source: values.stdin
        ? { kind: "stdin" }
        : { kind: "git", args: gitArgs, ...(includeUntracked ? { includeUntracked: true } : {}) },
      createdAt: now,
      updatedAt: now,
      revision: 0,
      seq: 0,
      cursor: { itemId: null, expanded: false },
      hunks: snapshot(patch),
      groups: [],
      queue: [],
      queueSet: false,
      applyReceipts: [],
  ...
  if (request.command === "apply") {
    let envelope;
    try {
      envelope = Schema.decodeUnknownSync(ApplyEnvelopeSchema)(JSON.parse(request.stdin ?? ""));
    } catch (error) {
      failure("validation_failed", "invalid apply envelope", [
        { opIndex: -1, message: String(error) },
      ]);
    }
  ...
  if (request.command === "refresh") {
    ...
    if (session.source.kind === "stdin") {
      ...
      const args = session.source.includeUntracked
        ? defaultGitArgs(session.repoRoot)
        : session.source.args;
