  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const apply = Effect.fn("Sessions.apply")(function* (request: Request) {
        ...
        const outcome = yield* Effect.fromResult(applyBatch(session, envelope));
        if (outcome.session) {
      ...
      const refresh = Effect.fn("Sessions.refresh")(function* (request: Request) {
        ...
        const refreshed = refreshSession(session, yield* Effect.fromResult(parseSnapshot(patch)));
      ...
      const tuiAction = Effect.fn("Sessions.tuiAction")(function* (request: Request) {
        ...
        const updated = yield* Effect.fromResult(applyHumanAction(session, request.action));
