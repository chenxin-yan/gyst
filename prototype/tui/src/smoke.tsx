...
assert(!onOneRow(t.captureCharFrame()), "auto resolves stack at width 100 (-/+ on separate rows)");
await t.mockInput.pressKey("1");
await t.renderOnce();
assert(onOneRow(t.captureCharFrame()), "forced split pairs deletion and addition on one row");
await t.mockInput.pressKey("0");
await t.renderOnce();
t.resize(140, 30);
await t.renderOnce();
assert(onOneRow(t.captureCharFrame()), "auto resolves split at width 140");
t.resize(100, 30);
await t.renderOnce();
assert(!onOneRow(t.captureCharFrame()), "auto falls back to stack when narrow");
