...
try {
  ...
  if (Bun.which("bun", { PATH: systemPath })) {
    throw new Error("package smoke PATH unexpectedly contains Bun");
  }
  // CLAUDE_CONFIG_DIR must not leak in: the skills startup repair would otherwise
  // rewrite the developer's real Claude skill links.
